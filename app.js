...
function loadState() {
  ...
  if (!raw) return { unlocked: [1], currentPhaseId: 1, currentMode: 'normal', studentName: '', attempts: {}, allAttempts: [], quizPassed: false, quizScore: 0 };
  try { return JSON.parse(raw); } catch { return { unlocked: [1], currentPhaseId: 1, currentMode: 'normal', studentName: '', attempts: {}, allAttempts: [], quizPassed: false, quizScore: 0 }; }
}
...
function escapeAttribute(text) { return String(text || '').replaceAll('&','&amp;').replaceAll('"','&quot;').replaceAll('<','&lt;').replaceAll('>','&gt;'); }
function escapeHtml(text) { return String(text).replaceAll('&','&amp;').replaceAll('<','&lt;').replaceAll('>','&gt;').replaceAll('"','&quot;').replaceAll("'",'&#039;').replaceAll(' ','&nbsp;').replaceAll('\n','<br>'); }
...
function getPracticeCollection(phase) { return practice.mode === 'english' ? phase.englishTexts : practice.mode === 'tables' ? phase.tableTexts : phase.texts; }
function getPracticeText() { const phase = getSelectedPhase(); const collection = getPracticeCollection(phase); return collection[practice.textIndex % collection.length]; }
  ...
  host.innerHTML = `
    <div class="hero">
      <div class="card">
        ...
        <p class="small">Esta versión incorpora práctica en <strong>texto alfanumérico</strong>, <strong>tablas sencillas</strong> y <strong>textos en inglés</strong>, además de seguimiento del alumnado y exportación de resultados en CSV.</p>
        <div class="actions">
        ...
        <p class="footer-note">Los datos se guardan en el navegador mediante <code class="inline">localStorage</code>. Puedes cambiar fases, textos, vídeos, preguntas y códigos desde <code class="inline">app.js</code>.</p>
      </div>
      <div class="card">
        <h2>Ruta de aprendizaje</h2>
        <div class="phase-list">
          ${phases.map(p => `<div class="phase ${state.unlocked.includes(p.id) ? '' : 'locked'}"><h3>${p.title}</h3><div class="small">Objetivo: <strong>${p.targetPPM} ppm</strong> · Precisión máxima: <strong>${p.maxErrorsPerMinute} falta/min</strong></div><div class="small">${p.prompt}</div><div class="small"><span class="pill">${state.unlocked.includes(p.id) ? 'Desbloqueada' : 'Bloqueada'}</span></div></div>`).join('')}
        </div>
      </div>
    </div>`;
...
function renderTargetText(typed) {
  ...
  document.getElementById('targetText').innerHTML = source.split('').map((ch, i) => {
    let cls = 'char';
...
function startTimer() {
  ...
  practice.timer = setInterval(() => {
    ...
    setText('timeValue', formatTime(practice.elapsedSeconds));
    updateStats(document.getElementById('typingInput')?.value || '', false);
  }, 250);
}

function registerAttempt(key, result) {
  if (!state.attempts[key]) state.attempts[key] = [];
  const enriched = { ...result, student: (practice.studentName || 'Sin nombre').trim(), phaseId: practice.phaseId, phaseTitle: getSelectedPhase().title, date: new Date().toLocaleString('es-ES') };
...
function showPracticeResult(result) {
  ...
  if (result.passed) {
    ...
    saveState();
    box.innerHTML = `<div class="message ok">¡Reto superado! Resultado: ${result.ppm} ppm, ${result.acc}% de precisión y ${result.errMin} faltas por minuto.<br>${next ? `Código de verificación para la siguiente fase: <strong>${next.code}</strong>` : 'Has completado todas las fases del itinerario.'}</div>`;
    renderHome();
  } else {
    box.innerHTML = `<div class="message bad">Intento no superado. Resultado: ${result.ppm} ppm y ${result.errMin} faltas/minuto. Debes alcanzar al menos ${phase.targetPPM} ppm y mantener como máximo ${phase.maxErrorsPerMinute} falta por minuto.</div>`;
  }
}
...
function updateStats(typed, finalize = true) {
  ...
  if (finishedText && !practice.completed && finalize) {
    ...
    registerAttempt(`phase-${practice.phaseId}-${practice.mode}`, { ppm, acc, errMin, errors, seconds, passed, mode: practice.mode });
...
function renderAttempts() {
  ...
  box.innerHTML = attempts.length ? attempts.map(a => `<div style="padding:10px 0; border-bottom:1px solid var(--line);"><strong>${a.passed ? 'Superada' : 'No superada'}</strong> · ${a.ppm} ppm · ${a.errMin} faltas/min · ${a.acc}% precisión<br><span class="small">${a.student} · ${a.date}</span></div>`).join('') : 'Todavía no hay intentos registrados en esta modalidad de la fase.';
}
...
function handleCodeUnlock() {
  ...
  if (!state.unlocked.includes(phase.id)) state.unlocked.push(phase.id);
  state.currentPhaseId = phase.id;
  practice.phaseId = phase.id;
  saveState();
  msg.innerHTML = `<div class="message ok">Código correcto. Se ha desbloqueado ${phase.title}.</div>`;
  renderHome();
  renderPractice();
}
...
function renderPractice() {
    ...
    <div class="grid grid-2">
      <div class="card">
        ...
        <div class="row">
          ...
          <div>
            <label>Fase</label>
            <select id="phaseSelect">${unlockedOptions.map(p => `<option value="${p.id}" ${p.id === phase.id ? 'selected' : ''}>${p.title}</option>`).join('')}</select>
          </div>
          <div>
      ...
      <div class="card">
        ...
        <div class="resource" style="margin-top:12px;">
          ...
          <p class="small">Puedes desbloquear una fase escribiendo el código que entregue el profesorado o el que se haya generado al superar la fase anterior.</p>
          <input id="codeInput" placeholder="Ejemplo: GA-MECA-100" />
  ...
  document.getElementById('studentName').addEventListener('input', e => { practice.studentName = e.target.value; state.studentName = practice.studentName; saveState(); });
  document.getElementById('phaseSelect').addEventListener('change', e => { practice.phaseId = Number(e.target.value); state.currentPhaseId = practice.phaseId; practice.textIndex = 0; saveState(); stopTimer(); renderPractice(); });
  document.getElementById('modeSelect').addEventListener('change', e => { practice.mode = e.target.value; state.currentMode = practice.mode; practice.textIndex = 0; saveState(); stopTimer(); renderPractice(); });
  ...
  document.getElementById('typingInput').addEventListener('input', e => { const typed = e.target.value; if (!practice.startedAt && typed.length > 0) startTimer(); renderTargetText(typed); updateStats(typed); });
  renderTargetText('');
  renderAttempts();
}
...
function renderTheory() {
  ...
  host.innerHTML = `
    ...
    <div class="grid grid-2" style="margin-top:18px;">
      ${theoryBlocks.map(block => `<div class="card"><h3>${block.title}</h3><p class="small">${block.summary}</p><div class="resource"><strong>Vídeo sugerido</strong><div style="position:relative; padding-top:56.25%; border-radius:16px; overflow:hidden; background:#e5eef8; margin-top:10px;"><iframe src="${block.embed}" title="${block.title}" style="position:absolute; inset:0; width:100%; height:100%; border:0;" allowfullscreen></iframe></div></div><div class="resource" style="margin-top:12px;"><strong>Recursos útiles</strong><ul>${block.external.map(r => `<li><a href="${r.url}" target="_blank" rel="noopener noreferrer">${r.label}</a></li>`).join('')}</ul></div></div>`).join('')}
    </div>`;
}
...
function renderTest() {
  ...
  host.innerHTML = `
    <div class="card"><h2>Test de autoevaluación</h2><p class="small">El estudiante visualiza los vídeos o recursos y después responde 10 preguntas.</p></div>
    <form id="quizForm" class="card" style="margin-top:18px;">${quiz.map((item, idx) => `<div class="question"><h4>${idx + 1}. ${item.q}</h4>${item.options.map((opt, i) => `<label><input type="radio" name="q${idx}" value="${i}" /> ${opt}</label>`).join('')}</div>`).join('')}<div class="actions"><button type="submit" class="button-main">Corregir test</button></div><div id="quizResult"></div></form>`;
  document.getElementById('quizForm').addEventListener('submit', e => {
    e.preventDefault();
    let score = 0;
    quiz.forEach((item, idx) => { const checked = document.querySelector(`input[name="q${idx}"]:checked`); if (checked && Number(checked.value) === item.answer) score += 1; });
...
function renderCriteria() {
  ...
  host.innerHTML = `
    <div class="card"><h2>Resultados de aprendizaje y criterios de evaluación</h2><p class="small">Bloque de referencia para alinear la práctica con la evaluación del módulo.</p></div>
    <div class="grid grid-2" style="margin-top:18px;">
      <div class="card"><h3>Contenidos básicos del módulo</h3><ul class="small"><li>Proceso de textos alfanuméricos en teclados extendidos.</li><li>Organización del tiempo y del espacio de trabajo.</li><li>Postura corporal ante el terminal.</li><li>Posición de brazos, muñecas y manos. Colocación de los dedos.</li><li>Riesgos laborales. Ergonomía en el puesto de trabajo.</li><li>Composición de un terminal informático: hardware y periféricos.</li><li>Software para el procesamiento de textos y reconocimiento de voz.</li><li>Conocimiento del teclado extendido.</li><li>Desarrollo de la destreza mecanográfica y corrección de errores.</li></ul></div>
      <div class="card"><h3>RA1 · Procesa textos alfanuméricos</h3><ul class="small"><li>Organización de los elementos y espacios de trabajo.</li><li>Mantenimiento de la posición corporal correcta.</li><li>Identificación de la posición correcta de los dedos.</li><li>Puesta en marcha del terminal informático.</li><li>Uso coordinado de líneas alfanuméricas, signos y puntuación.</li><li>Escritura al tacto en párrafos de dificultad progresiva y tablas sencillas.</li><li>Escritura al tacto para textos en inglés.</li><li>Velocidad mínima de 200 p.p.m. y máximo una falta por minuto.</li><li>Aplicación de normas de presentación de documentos.</li><li>Localización y corrección de errores mecanográficos.</li></ul></div>
    </div>`;
}
...
function renderTeacher() {
  ...
  host.innerHTML = `
    <div class="grid grid-2">
      <div class="card">
        <h2>Panel docente</h2>
        <p class="small">Códigos de acceso configurados por fases.</p>
        <div class="phase-list">${phases.map(p => `<div class="phase"><h3>${p.title}</h3><div class="small">Meta: ${p.targetPPM} ppm · Código: <strong>${p.code}</strong></div></div>`).join('')}</div>
      </div>
      <div class="card">
        <h2>Seguimiento del grupo y exportación</h2>
        <p class="small">Esta versión avanzada guarda intentos por alumno en el navegador y permite exportar los datos a CSV para hoja de cálculo.</p>
        <div class="actions">
        ...
        <p class="footer-note">Para personalizar la app, edita las fases, los textos, los vídeos, el test y los códigos desde <code class="inline">app.js</code>.</p>
      </div>
    </div>`;
  document.getElementById('exportAll').onclick = () => exportCSV('all');
  document.getElementById('exportSummary').onclick = () => exportCSV('summary');
  document.getElementById('resetProgress').onclick = () => { localStorage.removeItem(APP_KEY); location.reload(); };
}
